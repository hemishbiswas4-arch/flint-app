...
const LocationSearchInput = ({ value, onLocationSelect, isMapsScriptLoaded }: { value: string, onLocationSelect: (location: LocationState) => void, isMapsScriptLoaded: boolean }) => {
    const [inputValue, setInputValue] = useState(value);
    const autoCompleteRef = useRef<google.maps.places.Autocomplete | null>(null);
    const inputRef = useRef<HTMLInputElement | null>(null);

    useEffect(() => {
        if (isMapsScriptLoaded && window.google && inputRef.current && !autoCompleteRef.current) {
            autoCompleteRef.current = new window.google.maps.places.Autocomplete(inputRef.current);
            autoCompleteRef.current.addListener("place_changed", () => {
                const place = autoCompleteRef.current?.getPlace();
                if (place?.geometry?.location && place.formatted_address) {
                    onLocationSelect({ name: place.formatted_address, lat: place.geometry.location.lat(), lng: place.geometry.location.lng() });
                ...
                <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        {['Date', 'Friends', 'Solo', 'Family'].map(type => (
                            <div key={type} onClick={() => setFilter('groupType', type)} className={choiceCardClass(filters.groupType === type)}>
                                {type}
                            </div>
                        ))}
                    </div>
